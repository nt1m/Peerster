...
do
	...
	if [[ "$DEBUG" == "true" ]] ; then
		echo "$name running at UIPort $UIPort and gossipPort $gossipPort, peer: $peerPort"
	fi
	UIPort=$(($UIPort+1))
...
if !(grep -q "CLIENT MESSAGE $message" "E.out") ; then
	failed="T"
	echo -e "${RED}FAIL ${NC} client E.out"
fi

if !(grep -q "CLIENT MESSAGE $message2" "B.out") ; then
  failed="T"
	echo -e "${RED}FAIL ${NC} client B.out"
fi
...
do
	...
	if !(grep -q "$msgLine" "${outputFiles[$i]}") ; then
   		failed="T"
			echo -e "${RED}FAIL ${NC} check 1 in ${outputFiles[$i]}"
	fi
	if !(grep -q "$peersLine" "${outputFiles[$i]}") ; then
      failed="T"
			echo -e "${RED}FAIL ${NC} check 2 in ${outputFiles[$i]}"
  fi
	if !(grep -q "$msgLine2" "${outputFiles[$i]}") ; then
      failed="T"
			echo -e "${RED}FAIL ${NC} check 3 in ${outputFiles[$i]}"
  fi
done
